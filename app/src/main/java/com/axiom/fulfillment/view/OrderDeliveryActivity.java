...
public class OrderDeliveryActivity extends BaseActivity implements View.OnClickListener,
    ...
    private void apicall(DeliveryRequest req) {
        ...
        APIInterface apiService = new APIClient(this).getClientforPod().create(APIInterface.class);
            ...
            public void onResponse(Call<CommonApiResponse> call, Response<CommonApiResponse> response) {
                ...
                try {
                    ...
                    if (response.body()!=null && response.body().getStatus().getOutResult()) {
                        ...
                        ShowToast(jObjError.getJSONObject("Status").getString("OutMessage"), OrderDeliveryActivity.this);
                        Intent intent = getIntent();
                        intent.putExtra("order_status", "delivered");
                        setResult(RESULT_OK, intent);
                        finish();
                    ...
                    ShowToast("Server error, plz try again." , OrderDeliveryActivity.this);
...
//        startActivityForResult(intent, CAMERA);
 ...
 //           bitmap = Bitmap.createScaledBitmap(bitmap, Double.valueOf(bitmap.getWidth()/2).intValue(), Double.valueOf(bitmap.getHeight()/2).intValue(), true);
//            bitmap = Bitmap.createBitmap(bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.ARGB_8888);
    ...
    public Bitmap resizeGalleryImageNSave( String path) {
        ...
        try {
         ...
         //       bitmap = Bitmap.createScaledBitmap(bitmap, Double.valueOf(bitmap.getWidth()/1.5).intValue(), Double.valueOf(bitmap.getHeight()/1.5).intValue(), false);
